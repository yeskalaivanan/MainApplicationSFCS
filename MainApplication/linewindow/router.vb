Public Class router
    ...
    Private Sub btn_cancel_Click(sender As Object, e As EventArgs)
        result = check_serialnumber_touchupvi(txt_pcbsno.Text)
        If result = True Then
            update_router(txt_pcbsno.Text)
        Else
            MsgBox("touchup Stage not Passed")
        End If
    End Sub
...
nextline:
    ...
    Private Sub routersave()
        ...
        Else
            result = check_serialnumber_touchupvi(txt_pcbsno.Text)
            If result = True Then
                router_save(txt_pcbsno.Text, txt_product_code.Text, txt_product_name.Text, txt_lot_no.Text, txt_line_no.Text, Stage_aoi, cmb_status.SelectedItem, remarks)
            Else
                MsgBox("touchup Stage not Passed")
            End If
        End If
